...
corrdata<-read.table("stoltdata.txt", header=T)
treateddata<-corrdata[grep("^Stol", corrdata$naam),]
...
mTreat<-lm(treateddata$corrPercent~treateddata$leeftijdJaren+0)
r2All<-summary(mAll)$r.squared
r2Con<-summary(mCon)$r.squared
r2Treat<-summary(mTreat)$r.squared

plot(algdata$corrPercent~algdata$leeftijdJaren, pch=3, 
     main="Corrosion in ballast tanks",
     xlab="Age (years)", ylab="Percent corrosion")
points(treateddata$corrPercent, pch=3,col="red") 
legend("topleft", legend=c("Control", "Treated"),  pch=3,col=c("black", "red"))
abline(mCon, col="black")
abline(mTreat, col="red")
```
